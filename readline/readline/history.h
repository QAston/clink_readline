...
READLINE_API HIST_ENTRY **remove_history_range PARAMS((int, int));

/* Allocate a history entry consisting of STRING and TIMESTAMP and return
   a pointer to it. */
READLINE_API HIST_ENTRY *alloc_history_entry PARAMS((char *, char *));

/* Copy the history entry H, but not the (opaque) data pointer */
READLINE_API HIST_ENTRY *copy_history_entry PARAMS((HIST_ENTRY *));
...
READLINE_API int history_offset;

READLINE_API int history_lines_read_from_file;
READLINE_API int history_lines_written_to_file;
...
READLINE_API int history_quoting_state;

READLINE_API int history_write_timestamps;

/* These two are undocumented; the second is reserved for future use */
READLINE_API int history_multiline_entries;
READLINE_API int history_file_version;
